export function validateFieldConfig(field = {}) {
	...
	if (field.label.trim() === "") {
		field.labelError = "Label is empty";
	}

	switch (field.type) {
		case "section":
		case "choice": {
			field.fields = validateFieldConfigs(field.fields);
			break;
		}
		case "textarea": {
			if (field.rows <= 0) {
				field.rowsError = "Rows must be greater than 0";
			}

			if (field.cols <= 0) {
				field.colsError = "Columns must be greater than 0";
			}
